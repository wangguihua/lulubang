...
@interface RepadTwoViewController ()
{
...
@implementation RepadTwoViewController
...
-(void)getCodeBtnPressed{
    ...
    NSString *urlStr = [NSString stringWithFormat:@"%@?c=app&a=doCheckCode&m_mobile=%@&m_checkcode=%@",ROOTURLSTR,_phoneStr,MBNonEmptyStringNo_(_phoneTF.text)];
    NSString* encodedString = [urlStr stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    NSLog(@"%@",encodedString);
    ...
    [[HttpWorkHelp ShareData]httpGet:^(NSDictionary *dic) {
        ...
        NSLog(@"%@",dic);
        if ([MBNonEmptyStringNo_(dic[@"state"])isEqualToString:@"0"]) {
            RepadThreeViewController *threeSetp = [[RepadThreeViewController alloc]init];
