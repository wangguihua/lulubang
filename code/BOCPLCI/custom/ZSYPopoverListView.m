...
- (void)initTheInterface
{
    ...
    */
    ...
    self.mainPopoverListView.rowHeight = 55;
    [self addSubview:self.mainPopoverListView];
    [self setExtraCellLineHidden:self.mainPopoverListView];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!cell) {
        cell = [[[UITableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellInde] autorelease];
    }
    if (indexPath.row == 0) cell.textLabel.text = @"500米";
    if (indexPath.row == 1) cell.textLabel.text = @"1,000米";
    if (indexPath.row == 2) cell.textLabel.text = @"2,000米";
    if (indexPath.row == 3) cell.textLabel.text = @"5,000米";
    if (indexPath.row == 4) cell.textLabel.text = @"10,000米";
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row == 0) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:@"sysSetRange" object:@"500米"];
        [self animatedOut];

    }
    if (indexPath.row == 1) {
        
        [[NSNotificationCenter defaultCenter] postNotificationName:@"sysSetRange" object:@"1,000米"];
        [self animatedOut];

    }
    if (indexPath.row == 2) {
        
        [[NSNotificationCenter defaultCenter] postNotificationName:@"sysSetRange" object:@"2,000米"];
        [self animatedOut];
        
    }
    if (indexPath.row == 3) {
        
        [[NSNotificationCenter defaultCenter] postNotificationName:@"sysSetRange" object:@"5,000米"];
        [self animatedOut];
        
    }
    if (indexPath.row == 4) {
        
        [[NSNotificationCenter defaultCenter] postNotificationName:@"sysSetRange" object:@"10,000米"];
        [self animatedOut];
